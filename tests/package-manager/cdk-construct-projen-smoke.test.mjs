...
test('cdk-construct pins the JS-only pnpm, projen, cdk, and node floor', () => {
  ...
  assert.equal(packageJson.devDependencies['aws-cdk-lib'], '2.269.0');
  assert.equal(packageJson.devDependencies.constructs, '10.5.1');
  assert.equal(packageJson.devDependencies.jsii, '~5.9.54');
  assert.equal(packageJson.devDependencies['jsii-docgen'], '^10.12.6');
  assert.equal(packageJson.peerDependencies['aws-cdk-lib'], '^2.269.0');
